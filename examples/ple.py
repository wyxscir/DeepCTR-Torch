...
class PLE(BaseModel):
    ...
    def __init__(self, dnn_feature_columns, shared_expert_num=1, specific_expert_num=1, num_levels=2,
        ...
        def multi_module_list(num_level, num_tasks, expert_num, inputs_dim_level0, inputs_dim_not_level0, hidden_units):
            return nn.ModuleList(  # 模型的基本结构也是DNN
        ...
        # 1. experts
        # task-specific experts 霍 他这是一次把要用到所有小专家都初始化了，用一个三维的nn.ModuleList 存起来
        ...
        # 2. gates
        # gates for task-specific experts gate同样也要初始化那么多
    ...
    def forward(self, X):
        ...
        # repeat `dnn_input` for several times to generate cgc input  同一份输入分成这么多份 送给每一个专家网络
        ple_inputs = [dnn_input] * (self.num_tasks + 1)  # [task1, task2, ... taskn, shared task]  用一个列表的形式存储
        ple_outputs = []
        for i in range(self.num_levels):  # 这个层是一层专家网络 就和一层的transformer是一样的
